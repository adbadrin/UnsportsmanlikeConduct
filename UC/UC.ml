...
let facebook_login =
  Html5.F.Raw.a 
    ~a:[a_href (Xml.uri_of_string ("https://www.facebook.com/dialog/oauth?client_id=" ^ 
                                   facebook_app_id ^ "&redirect_uri=http://localhost:8080/login"))]
      [pcdata "Login with Facebook"]
(*
  Eliom_content.Html5.D.a
    (Eliom_service.Http.external_service
      ~prefix:("https://www.facebook.com/dialog/oauth?client_id=" ^ 
               facebook_app_id ^ "&redirect_uri=http://localhost:8080/login")
      ~path:["facebook_login"]
...
let subtitle =
  "Play with your friends or play with strangers!"
